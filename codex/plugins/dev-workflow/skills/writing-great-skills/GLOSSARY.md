...
A skill eligible for automatic selection through its description. Keep both required metadata fields, name and description. Invocation policy is configured through the host; in Codex automatic selection is the default unless agents/openai.yaml disables it.

_Avoid_: ability, tool, capability

### User-Invoked

A skill configured for explicit invocation through host policy. Keep its required description. In Codex set policy.allow_implicit_invocation: false in agents/openai.yaml; Claude uses disable-model-invocation: true. Context visibility and cross-skill reach depend on the host, not on deleting the description.

_Avoid_: procedure, workflow, command

### Description

The required discovery summary describing the capability and the requests it applies to. It informs automatic matching but does not itself set invocation policy. Keep triggers specific and distinguish nearby capabilities.
...
The metadata and instructions a host places in the agent's context. Automatic skill discovery has a cost in tokens and attention; explicit-only discovery can reduce that cost depending on the host. Measure the actual loaded context rather than inferring it from the presence of a description.
...
A skill that maps requests to focused skills. The router does not change their host invocation policy or grant permission for their external actions.
...
**Reference** stored outside a skill entrypoint, read through an explicit path when needed. It is a useful home for shared contracts regardless of invocation policy. Distribute it with its callers and define when reading it is required.

#!/bin/bash
# Install the post-commit validation gate in the current repository.
set -euo pipefail
...
for name in pre-commit pre-push; do
    target="$HOOKS/$name"
    if [ -f "$target" ] && ! grep -qE 'Claude Code 개발 플로우|Ultivis workflow|Ultivis validation' "$target"; then
...
mkdir -p "$ROOT/tmp"
echo "설치 완료: commit → validation → push. pre-commit은 staged diff, pre-push는 현재 커밋의 검증 결과를 검사합니다."

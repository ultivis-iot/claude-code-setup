...
#
# 명시적으로 연결된 WORKFLOW_APPROVED_PLAN만 복사합니다.
# 전역 plans의 수정 시각은 현재 세션/승인의 식별자가 아닙니다.
...
CWD=$(git -C "$CWD" rev-parse --show-toplevel 2>/dev/null || printf '%s\n' "$CWD")

# 호출 환경이 승인된 정확한 경로를 제공한 경우에만 자동 복사한다.
APPROVED_PLAN="${WORKFLOW_APPROVED_PLAN:-}"
if [ -n "$APPROVED_PLAN" ] && [[ "$APPROVED_PLAN" != /* ]]; then
    echo '{"decision": "approve", "reason": "WORKFLOW_APPROVED_PLAN에는 승인된 Plan의 절대 경로가 필요합니다. 자동 복사를 생략합니다."}'
    exit 0
fi

if [ -z "$APPROVED_PLAN" ] || [ ! -f "$APPROVED_PLAN" ]; then
    echo '{"decision": "approve", "reason": "현재 세션에 연결된 Plan 경로가 없습니다. 현재 대화에서 승인된 정확한 Plan을 tmp/current-plan.md로 저장하세요. 전역 최신 Plan을 추측해서 복사하지 마세요."}'
    exit 0
fi

# Plan 내용 읽기
PLAN_CONTENT=$(cat "$APPROVED_PLAN")
...
Plan 성격에 따라 추천:
  ...
  • **현재 Task의 보완/메모** → \`/ult-task-note '메모'\`
    → 현재 Task에 코멘트만 추가, 새 Task 안 만듦

  • **현재 Story에 관련 Task 추가** → \`/ult-task-create --here\`
...
Plan 성격에 따라 추천:
  ...
  • **거대한 Plan (새 사용자 가치, 여러 단계)** → \`/ult-story-create\`
    → 새 Story + Task N개 + Github Issue N개 + 브랜치 N개 일괄 생성

  • **기존 Story의 추가 작업** → \`/ult-task-create <story>\`
